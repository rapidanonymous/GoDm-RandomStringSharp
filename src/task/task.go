...
func MassDmTask(in []modules.Instance, msg string, randomAmount string, interval time.Duration) {
	cfg, _ := Mod.LoadConfig("config.json")
		...
		go func(i int, c modules.Instance) {
			...
			for j := 0; j < len(uid); j++ {
				...
				if c.Eligible(uid[j]) {
					...
					resp, body := c.Message(msg, data.Id, randomAmount, modules.MessageOptions{Loop: false})
						...
						if strings.Contains(string(body), "captcha_sitekey") {
							if !cfg.Mode.Configs.Solver {
								...
								for count := 0; count < cfg.Mode.Configs.CaptchaRetry; {
									count++
									resp, body = c.Message(msg, data.Id, randomAmount,
