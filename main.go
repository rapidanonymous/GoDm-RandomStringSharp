	...
	opt := FuncMap{
		1: func() {
			...
			r:
				...
				randomAmount := Mod.Input(modules.RandomAmountInput)
				task.MassDmTask(in, msg, randomAmount, cooldown)
			case 2:
		...
		2: func() {
			...
			task.StartTask(in, func(c modules.Instance) {
				...
				if c.Eligible(ID) {
					c.Message(msg, data.Id, randomAmount, modules.MessageOptions{Loop: true})
				}
			})
		},
		3: func() {
		...
		7: func() {
			...
			randomAmount := Mod.Input(modules.RandomAmountInput)
			task.StartTask(in, func(c modules.Instance) {
				c.Message(msg, ID, randomAmount, modules.MessageOptions{Loop: true})
			})
		},
		8: func() {
		...
		10: func() {
			...
			task.StartTask(in, func(c modules.Instance) {
				c.Message(msg, ID, randomAmount, options)
			})
		},
		11: func() {
